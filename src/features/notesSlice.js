...
export const fetchNotes = createAsyncThunk("fetchNotes", async () => {
  const response = await fetch("https://notes-backend-i0bu.onrender.com/notes/getnotes",{
    method: "GET",
    headers: {
...
export const sendNotes = createAsyncThunk("sendNotes", async (tempNote) => {
  const response = fetch("https://notes-backend-i0bu.onrender.com/notes/addnote", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${localStorage.getItem("token")}`
